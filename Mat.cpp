...
void Mat::Initialize(int _numRowSubarray, int _numColumnSubarray, int _numAddressBit, long _numDataBit,
	...
	if (numRow <16||numRow > 512) {
		invalid = true;
		cout << "[Mat]: Word width is impractical." << endl;
		initialized = true;
		return;
	}
	// TO-DO
	...
	if (numColumn <16||numColumn > 512) {
		invalid = true;
		cout << "[Mat]: Column width is impractical." << endl;
		initialized = true;
		return;
	}
	subarray.Initialize(numRow, numColumn, numRowPerSet > 1, true /* TO-DO: need to correct */,
				muxSenseAmp, internalSenseAmp, muxOutputLev1, muxOutputLev2,
				areaOptimizationLevel, (BufferDesignTarget)CAM_opt.RowDriver,
				inputParameter->withInputEnc, inputParameter->typeInputEnc,  inputParameter->customInputEnc,
				inputParameter->typeSenseAmp,  inputParameter->customSenseAmp, inputParameter->withWriteDriver,
				inputParameter->withOutputAcc, inputParameter->withPriorityEnc,  (BufferDesignTarget)CAM_opt.Proirity,
				inputParameter->withInputBuffer, inputParameter->withOutputBuffer, inputParameter->camType, inputParameter->searchFunction);
	...
	if( inputParameter->designTarget == CAM_chip) {
			numAddressRowPredecoderBlock1 = _numAddressBit + (int)(!CAM_cell->camMLC) - (int)(log2(muxSenseAmp)+0.1);	/* The address bit on row decodeing */
	}
